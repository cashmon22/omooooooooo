...
import type { CreatePaymentRequestInput, PaymentRequestStatus } from "../../shared/payment-requests";

const allowedStatuses: PaymentRequestStatus[] = ["Pending Review", "Approved", "Rejected", "Completed"];
...
export const createPaymentRequest: RequestHandler = async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .select("id, user_id, full_legal_name, email, phone, delivery_address, city, state_province, postal_code, country, device_id, device_name, device_model, device_amount, currency, vendor, status, created_at")
    .single();
...
export const listPaymentRequests: RequestHandler = async (req, res) => {
  ...
  const { data, error } = isAdmin(user) ? await query : await query.eq("user_id", user.id);
