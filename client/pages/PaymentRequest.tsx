    ...
    <section className="rounded-xl border border-emerald-200 bg-white p-6 shadow-card sm:p-10">
      ...
      <div className="mt-9 border-t border-slate-100 pt-8">
        ...
        <div className="mt-6 space-y-5">
          {[['1. Payment Processing', 'A payment representing the selected device amount will be provided through the approved payment process.'], ['2. Payment Receipt', 'Follow the verified payment instructions provided to you through the official process.'], ['3. Vendor Payment', 'Once payment is legitimately available, follow the approved instructions for completing payment to the selected vendor.']].map(([title, detail]) => <div key={title} className="flex gap-3"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-orange" /><div><h3 className="text-sm font-extrabold text-navy">{title}</h3><p className="mt-1 text-sm leading-6 text-slate-500">{detail}</p></div></div>)}
        </div>
      </div>
